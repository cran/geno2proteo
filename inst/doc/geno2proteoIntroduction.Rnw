...
uses to define the genomic locus and the protein section, which are the input 
of these functions.
...
\item \Rfunction{genomicLocToProteinSequence} takes a list of genomic regions 
...
\Rfunction{genomicLocToProteinSequence} by using the CDSs data file generated 
by the codes in Section \ref{sectDataFiles} and some genomic loci defined by 
their coordinates.

\small
<<example_genomicLocToProteinSequence, eval=TRUE>>=
library(geno2proteo)
...
\normalsize
\item \Rfunction{genomicLocToWholeDNASequence} takes a list of genomic regions
and tries to find the DNA sequences of those genomic regions. Note that, as 
explained above, another function \Rfunction{genomicLocToProteinSequence} 
...
\Rfunction{genomicLocToProteinSequence}, which has been explained in the above.
...
The following codes illustrate the usage of the function 
\Rfunction{genomicLocToWholeDNASequence} by using the CDSs data file generated
by the codes in Section \ref{sectDataFiles} and some genomic loci defined by 
their coordinates.

\small
<<example_genomicLocToWholeDNASequence, eval=TRUE>>=
library(geno2proteo)
...
%\Rfunction{genomicLocToWholeDNASequence}.
...
\Rfunction{genomicLocToProteinSequence}. 
\end{itemize}

The function returns a data frame containing the original protein sections 
specified in the input and before them, the six added columns for the genomic 
positions of the protein sections:
...
corresponding to the protein section.
...
sequences of the protein sections.
